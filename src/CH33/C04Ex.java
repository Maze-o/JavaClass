...
public class C04Ex {
	public static Function<List<String>, Integer> func1 = x -> (int) x.stream().filter(el -> el.length() >= 5).count();
	public static Function<List<Integer>, Integer> func2 = x -> x.stream().reduce(0, (z, y) -> z + (y * y));
	public static Function<List<String>, List<String>> func3 = x -> x.stream().map(s -> String.valueOf(Character.toUpperCase(s.charAt(0)))).collect(Collectors.toList());
